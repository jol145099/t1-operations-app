...
function PlayerCard(p:any){
 ...
 return <Card>
  ...
  <Muted>{displayPay(r)<0?'賠付':'實拿金額'}：{formatMoney(displayPay(r))}</Muted>
...
function formatMoney(v:any){const n=Number(v||0);return n<0?`-${Math.abs(n).toLocaleString()}`:`${n.toLocaleString()}`}
