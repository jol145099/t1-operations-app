...
export default function OrderDetailScreen(){
 ...
 const [replaceId,setReplaceId]=useState<string|null>(null),[newPlayerId,setNewPlayerId]=useState(''),[playerQuery,setPlayerQuery]=useState(''),[finalPay,setFinalPay]=useState(''),[comp,setComp]=useState(''),[reason,setReason]=useState(''),[completeDate,setCompleteDate]=useState(todayLocal())
 ...
 async function replacePlayer(){
  ...
  const {data:created,error:insertError}=await supabase.from('order_players').insert({
   order_id:orderId,player_id:newPlayerId,status:'in_progress',assigned_pay:0,calculated_pay:0,final_pay:0,is_active_slot:true
  }).select('id').single()
