...
type SettlementSummary = { player: Player; orderPay: number; dispatchPay: number; adjustments: number; total: number }
...
export default function SettlementScreen() {
  ...
  function openDetails(summary: SettlementSummary) {
    router.push({
      pathname: '/settlement-detail',
      params: {
        playerId: summary.player.id,
        playerName: summary.player.display_name,
        start: period.start.toISOString(),
        end: period.end.toISOString(),
        nextExclusive: period.nextExclusive.toISOString()
      }
    })
  }
    ...
    <ScrollView style={{ flex: 1, backgroundColor: colors.bg }} contentContainerStyle={{ flexGrow: 1 }}>
      <Screen>
        ...
        {summaries.map((summary) => (
          <TouchableOpacity key={summary.player.id} onPress={() => openDetails(summary)} activeOpacity={0.75}>
            <Card>
