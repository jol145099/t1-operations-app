...
export default function SettlementScreen(){
 ...
 const [loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[error,setError]=useState(''),[selectedPeriod,setSelectedPeriod]=useState('current'),[confirmationOpen,setConfirmationOpen]=useState(false)
 ...
 async function load(){
  ...
  const [p,a,l,d,s,h,g]=await Promise.all([
   pq,
 ...
 async function toggleOpen(){setSaving(true);const {error:e}=await supabase.rpc('set_t1_settlement_confirmation_open',{p_period_start:dateOnly(period.start),p_period_end:dateOnly(period.end),p_open:!confirmationOpen});setSaving(false);if(e)Alert.alert('更新失敗',e.message);else await load()}
 ...
 return <ScrollView style={{flex:1,backgroundColor:colors.bg}}><Screen><H1>薪資結算</H1>
  <Card><H2>本期：{label(period.start)} → {label(period.end)}</H2><Muted>{confirmationOpen?'客服已檢查完成，打手現在可以確認本期結算。':'客服尚未開放結算確認。'}</Muted>{canManage?<Button title={confirmationOpen?'關閉打手確認':'客服檢查完成・開放統一結算'} onPress={toggleOpen} disabled={saving} tone={confirmationOpen?'neutral':undefined}/>:null}</Card>
