...
export default function SettlementScreen(){
 ...
 const [loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[error,setError]=useState(''),[historyOpen,setHistoryOpen]=useState(false)
 ...
 return <ScrollView style={{flex:1,backgroundColor:colors.bg}}><Screen><H1>薪資結算</H1>
  ...
  <Button title={historyOpen?'收起過去結算':'查看過去結算'} tone="neutral" onPress={()=>setHistoryOpen(!historyOpen)}/>
  {historyOpen?<Card><H2>過去結算</H2>{history.length===0?<Muted>尚無歷史紀錄。</Muted>:history.map(x=><TouchableOpacity key={x.id} onPress={()=>router.push({pathname:'/settlement-detail',params:{playerId:x.player_id,playerName:x.players?.display_name||'打手',start:new Date(x.period_start+'T00:00:00').toISOString(),end:new Date(x.period_end+'T00:00:00').toISOString(),nextExclusive:new Date(new Date(x.period_end+'T00:00:00').getTime()+86400000).toISOString(),carryIn:String(x.carry_in||0),savedTotal:String(x.total_payable)}})}><View style={styles.history}><View><Text style={styles.name}>{x.players?.display_name||'打手'}</Text><Muted>{x.period_start} → {x.period_end}</Muted></View><View><Text style={x.total_payable<0?styles.negative:styles.amount}>{signed(Number(x.total_payable))}</Text><Muted>{x.payment_status==='paid'?'已付款':x.total_payable<0?'結轉下期':'待付款'}</Muted></View></View></TouchableOpacity>)}</Card>:null}
  {error?<Card><Text style={styles.negative}>{error}</Text></Card>:null}
  {summaries.map(x=><View key={x.player.id}><TouchableOpacity onPress={()=>detail(x)}><Card><View style={styles.header}><H2>{x.player.display_name}</H2><Text style={x.saved?styles.saved:styles.live}>{x.saved?'已確認':'即時計算'}</Text></View><Line label="完單收入" value={x.orderPay}/><Line label="派單收入" value={x.dispatchPay}/><Line label="其他加扣" value={x.adjustments}/>{x.carryIn!==0?<Line label="上期結轉" value={x.carryIn}/>:null}<View style={styles.divider}/><Line label={x.total<0?'結轉下期':'本期應付'} value={x.total} total/></Card></TouchableOpacity>{canManage?<View style={styles.actions}>{!x.saved?<Button title="確認結算" onPress={()=>confirm(x)} disabled={saving}/>:x.total>0&&x.saved.payment_status!=='paid'?<Button title={`標記已付款 ${money(x.total)}`} onPress={()=>pay(x)} disabled={saving}/>:null}</View>:null}</View>)}
 </Screen></ScrollView>
}
